...
module.exports = function (req) {
  ...
  req.connection.on(enums.PACKET_TYPES.LOBBY_STAT_ALTER, (packet, next) => {
    ...
    if (!req.player.getPlayerSkill(packet.skill)) return handleError(req, error.messages.INVALID_SKILL);

    // if (!req.player.hasAvailableSkillPoints()) return handleError(req, error.messages.NO_SKILL_POINTS_AVAILABLE);
