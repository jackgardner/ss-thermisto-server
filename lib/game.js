...
class Connection {
  ...
  startConnection() {
    // if we havent got an account, just return here (this should never hit because we're authorizing at an earlier point.)
    var account_id = accounts.find(acc => acc.email === this.connection.handshake.decoded_token._id)
    if (!account_id) {
      return;
    }

    // I.e we're a returning player.
    this.player = this.game.state.players.find(player => player.account_id === account_id);

    if (!this.player){
      // If we're not a returning player, then lets make a new player.
      this.player = new Player(randomId(12), account_id);
